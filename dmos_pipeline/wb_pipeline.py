...
class WBpipeline:
    ...
    Walking bouts are extracted from the temporal structure of detected gait
    events. Left and right foot events are first processed separately to identify
    inter-stride gaps. These gap signals are then combined to detect periods
    where gait events are absent from both feet simultaneously. Long simultaneous
    no-event intervals are classified as walking-bout pauses, while shorter
    intervals are retained as signal breaks.

    Valid walking bouts are obtained by splitting the recording at detected
    pauses, assigning left and right stride IDs to each candidate segment, and
    applying configurable stride-count rules inspired by the Mobilise-D walking
    bout definition.

    If event-level quality-control labels are available, the class can
    optionally exclude events that did not pass the quality check before walking
    bout extraction.

    The pipeline saves:
    - detected walking-bout pauses,
    - short signal breaks,
    - all candidate segments with validity labels,
    - final valid walking bouts.

    Notes
    -----
    This class does not compute gait events itself. It relies on event and log
    files generated by the previous gait-processing steps.
    """
