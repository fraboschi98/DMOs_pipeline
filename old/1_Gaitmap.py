...
class GaitMapPipeline:
    ...
    def plot_gs_events(
        ...
        """
        Visualize IMU signals together with detected gait sequences, strides,
        gait events, and optionally removed events.
    
        The method generates one subplot for each sensor side and overlays
        gait-analysis outputs on the selected signal channel. Depending on the
        selected options, the figure can display:
        - detected gait sequences,
        - segmented strides,
        - valid gait events,
        - stride events removed during spatial-parameter cleanup.
        ...
        ------------------
        Signal:
            Raw or filtered IMU signal.
    
        Gait sequences:
            Shaded light-blue regions corresponding to detected gait sequences.
    
        Strides:
            Shaded light-green regions corresponding to segmented strides.
            Stride start/end boundaries are displayed as vertical lines.
    
        Valid events:
            - `pre_ic`: green markers
            - `ic`: red markers
    
        Removed events:
            - removed `pre_ic`: orange cross markers
            - removed `ic`: purple cross markers
    
        Parameters
        ----------
        channel : str, default="gyr_ml"
            IMU channel to visualize.
    
        start : int, optional
            First sample index to display.
    
        end : int, optional
            Last sample index to display.
    
        signal_type : {"filtered", "raw"}, default="filtered"
            Signal source used for visualization.
    
        show_events : bool, default=True
            If True, overlay valid gait events.
    
        show_removed_events : bool, default=False
            If True, overlay events removed during spatial-parameter cleanup.
    
        show_strides : bool, default=False
            If True, display stride segmentation instead of gait-sequence regions.
    
        events_attr : str, default="events"
            Attribute name containing valid gait events.
    
        removed_events_attr : str, default="removed_events"
            Attribute name containing removed gait events.
    
        strides_attr : str, default="stride_list"
            Attribute name containing segmented strides.
        ...
        ------
        ValueError
            If required signals or gait-analysis outputs are missing.
    
        KeyError
            If requested channels or sensor sides are unavailable.
    
        Notes
        -----
        This visualization method is mainly intended for quality control and
        debugging of gait detection, stride segmentation, and event detection
        on real-world IMU recordings.
        """
